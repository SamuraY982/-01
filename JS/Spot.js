...
let priceDirection = "auto";   // up | down | auto
let smoothSpeed = 2;        // 🔥 0.01–0.03 (чем меньше — тем плавнее)
let candleLife = 10;            // сколько тиков живёт свеча
let candleMaxLife = 30;        // сколько тиков на одну свечу
...
function updatePriceOnly() {
  // направление
  let dir = 0;
  if (priceDirection === "up") dir = 1;
  else if (priceDirection === "down") dir = -1;
  else dir = Math.random() - 0.5;

  // микро-движение (очень маленькое)
  const microMove = last * 0.00015 * dir * smoothSpeed;

  // следующая цена
  const next = last + microMove;

  // обновляем ТЕКУЩУЮ свечу
  const cur = candles[candles.length - 1];
  cur.c += (next - cur.c);        // микро шаг
  cur.h = Math.max(cur.h, cur.c);
  cur.l = Math.min(cur.l, cur.c);
  cur.v += 0.4 + Math.random() * 0.6;

  last = cur.c;

  candleLife++;

  // создаём новую свечу РЕДКО и МЯГКО
  if (candleLife >= candleMaxLife) {
    candleLife = 0;
